...
// 正整数快速幂并取模 base^exp (mod MOD)
// 思路: a^10 = a^(0b1010) = (a^1)^0 * (a^2)^1 * (a^4)^0 * (a^8)^1
template <typename T1, typename T2, typename T3>
T1 powerMod(T2 base, T3 exp, T1 MOD) {
    T1 res = 1;
    base %= MOD;
    assert(exp >= 0); // 如果 exp < 0，则终止程序执行
...
ul combine(ul n) {
    ...
    ul x = (1 << n) + 1; // x 值为 2^n + 1
    ul mask = (1 << n) - 1; // mask 掩码值为 2^n - 1，二进制值为n个1
    
    // 注意：power(x, n) 很有溢出风险！！！！！:(
    // power(x, n) 即 (2^n + 1)^n，而 >> ((n >> 1) * n)) 表示除以 2^(n*n/2)，即右移 (2^n)^(n/2)
    // 基数为 2^n，需转换：按位与 & mask 表示只取低二进制 n 位的数值，这就是系数 C(n,n/2) 的真实值
    return (power<ul, ul, ul>(x, n) >> ((n >> 1) * n)) & mask;
}


// 求阶乘
ul fastFactorial(ul n) {
    ul temp;
