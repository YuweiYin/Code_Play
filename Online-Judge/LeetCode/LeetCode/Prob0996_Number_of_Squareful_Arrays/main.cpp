...
private:
    ...
    // 方法一：库函数，暴力查找，超时。时间复杂度 O(N!)，空间复杂度 O(1)。N 为 A_len
    ...
    // 方法二。回溯法。时间复杂度 O(N^N) 剪枝效果好，空间复杂度 O(1)。N 为 A_len
    // 思路：构造一张图，有 A 中所有的结点，以及一些边 x-y，如果 A[x] + A[y] 是完全平方数
...
//        }
    ...
    // 方法三。动态规划。时间复杂度 O(N*2^N) 剪枝效果好，空间复杂度 O(N*2^N)。N 为 A_len
    // 因为结点的数量很少(题目规定不超过 12 个)，所以可以使用 visited 数组标记所有已经过的结点，剪枝
};
