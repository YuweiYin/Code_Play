...
class Solution:
    ...
    def find_path(self, root, expect_number):
        # 思路：深度优先搜索
        # 剪枝条件：当前路径结点值总和超过了 expect_number
        ...
        if root.val <= expect_number and root.left is not None:
            self.find_path(root.left, expect_number - root.val)

        # 若右子树不为空，且值小于目标值，则递归遍历下一层
        if root.val <= expect_number and root.right is not None:
            self.find_path(root.right, expect_number - root.val)
        ...
        #     # 若当前路径累积值已达到预期值
        #     if self.path_len == expect_number and root.left is None and root.right is None:
        ...
        #         # self.find_path(father, expect_number)
        #     else:
        #         # 不应出现的情况
        #         pass
        #
        ...
        #         self.find_path(self.path_list_current[len_list - 1].left, expect_number)
        #     if root.right is not None:
        #         print '向右走', self.path_list_current[len_list - 1]
        #         self.find_path(self.path_list_current[len_list - 1].right, expect_number)
        #
        # # return None
...
def main():
    ...
    res = solution.find_path(root, expect_number)
