...
![shortest-path-1](./img/shortest-path-1.png)
...
- 结点集 $V_p = \{ v \in V: v.p != nil \} \cup \{ s \}$，即所有前驱结点不为 nil 的结点 以及源结点 s。
- 边集 $E_p = \{ (v.p, v) \in E: v \in Vp - \{ s \} \}$，即由 Vp 中结点的 p 值所诱导的边的集合。
...
![shortest-path-2](./img/shortest-path-2.png)
...
![shortest-path-3](./img/shortest-path-3.png)
...
- **路径松弛性质**（引理 24.15）：如果 `p = <v0, v1, ..., vk>` 是从源结点 s = v0 到结点 vk 的一条最短路径，并且对 p 中的边所进行松弛的次序为 $(v0, v1), (v1, v2), ..., (v_{k-1}, v_{k})$，则 vk.d = d(s, vk)。意味着结点 vk 找到了最短路。
...
- 对于 Dijkstra 算法
	- 如果使用**线性数组**来实现**最小优先队列** Q，则 $|V|$ 次运行的总时间是 $O(|V|^3 + |V| \cdot |E|)$，即 $O(|V|^3)$。
	- 如果使用**二叉堆**实现 Q，则 $|V|$ 次运行的总时间是 $O(|V| \cdot |E| log |V|)$，这在**稀疏图**的情况下是个较大的改进。
	- 如果使用**斐波那契堆**实现 Q，则 $|V|$ 次运行的总时间是 $O(|V| \cdot |E| + |V|^2 log |V|)$。这对大型稀疏图来说是个很好的算法。

但是如果图中含有权重为负值的边，就不能使用 Dijkstra 算法。此时如果运行 Bellman-Ford 算法，$|V|$ 次运行的总时间是 $O(|V|^2 \cdot |E|)$，在稠密图（$|E|$ 接近 $|V|^2$）的情况下，此运行时间为 $O(|V|^4)$。
